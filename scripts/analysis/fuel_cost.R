...
if (exists("snakemake")) {
  ...
  choice_out      <- here("data", "fuel_cost_choice_emm.csv")
  rating_out      <- here("data", "fuel_cost_rating_emm.csv")
}
...
df <- conjoint |>
  ...
  mutate(
    ...
    # Sustainable aviation fuels (biofuels and synthetic/e-fuels) collapsed
    # into one category, contrasted against fossil fuels - a coarser binary
    # cut of the 3-level fuel attribute, analogous to durability's
    # Temporary/Permanent comparison.
    fuel_binary = case_when(
      fuel_code == "fossil" ~ "Fossil fuels",
      fuel_code %in% c("plants", "electric") ~ "SAFs",
      TRUE ~ NA_character_
    ),
...
# interest (fuel x cost x country) is left fully crossed so the cost/fuel
# trade-off can vary by country.
...
choice_model <- suppressWarnings(
  glmer(
    binary_choice ~
      activity_code + durability_code + responsibility_code +
      fuel_binary * cost_code * country +
      (1 | id),
    data = df,
    family = binomial,
...
choice_emm <- emmeans(
  ...
  as.data.frame() |>
  select(country, fuel_binary, cost_code, prob, SE, df, asymp.LCL, asymp.UCL)
...
rating_emm <- emmeans(
  ...
  select(country, fuel_binary, cost_code, emmean, SE, df, lower.CL, upper.CL)

write_csv(rating_emm, rating_out)
ts_msg("Rating emmeans written (", elapsed(t1), "s total).")

message("Fuel x cost interaction analysis completed:")
